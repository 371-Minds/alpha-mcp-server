 ...
 */
function generateJWT(method: string, path: string): string {
...
function requireMarketCreds(): McpResult | null {
...
const definitions: ToolDef[] = [
  ...
  },
  {
    ...
    description:
      "Place a market or limit order on Coinbase Advanced Trade. Requires COINBASE_ADV_TRADE_API_KEY and COINBASE_ADV_TRADE_SECRET.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Cancel one or more open orders by ID. Requires COINBASE_ADV_TRADE_API_KEY and COINBASE_ADV_TRADE_SECRET.",
    inputSchema: {
      type: "object",
      properties: {
        order_ids: {
  ...
  },
  {
    ...
    description:
      "Get trade execution history (fills) for your orders. Requires COINBASE_ADV_TRADE_API_KEY and COINBASE_ADV_TRADE_SECRET.",
    inputSchema: {
      type: "object",
      properties: {
    ...
    switch (name) {
      ...
      case "market_list_products": {
        ...
        const lines = products.map(
          (p) => `  ${(p.product_id as string).padEnd(12)} | ${String(p.price || "—").padStart(14)} | ${p.status}`
        );
      ...
      case "market_get_candles": {
        ...
        } = args as { product_id: string; granularity?: string; start?: string; end?: string };
        const now = Math.floor(Date.now() / 1000);
        const startTs = start || String(now - 86400);
        const endTs = end || String(now);
        const data = await advTradeCall(
          `/api/v3/brokerage/products/${product_id}/candles?granularity=${granularity}&start=${startTs}&end=${endTs}`,
          "GET"
        );
        const candles = (data.candles as Array<Record<string, string>>) || [];
      ...
      case "market_get_orderbook": {
        ...
        const book = data.pricebook as Record<string, unknown>;
        const bids = (book?.bids as Array<Record<string, string>>) || [];
        const asks = (book?.asks as Array<Record<string, string>>) || [];
      ...
      case "market_create_order": {
        ...
        const { product_id, side, order_type, base_size, quote_size, limit_price, client_order_id } = args as {
          product_id: string;
          side: string;
          order_type: string;
          base_size?: string;
        ...
        if (order_type === "market") {
          ...
          if (!limit_price) return err("limit_price is required for limit orders");
          if (!base_size) return err("base_size is required for limit orders");
          orderConfig = {
            limit_limit_gtc: {
              base_size,
              limit_price,
        ...
        const order = data.success_response as Record<string, unknown> | undefined;
        if (!order) {
      ...
      case "market_list_orders": {
        const credsErr = requireMarketCreds();
        if (credsErr) return credsErr;
        const { product_id, order_status = ["OPEN"], limit = 25 } = args as {
          product_id?: string;
          order_status?: string[];
          limit?: number;
        };
        ...
        const data = await advTradeCall(
          `/api/v3/brokerage/orders/historical/batch?${statusParams}${productParam}&limit=${limit}`,
          "GET"
        );
        const orders = (data.orders as Array<Record<string, unknown>>) || [];
        if (orders.length === 0) return ok("No orders found.");
        const lines = orders.map(
          (o) =>
            `  ${o.order_id} | ${o.product_id} | ${o.side} | ${o.status} | size:${o.base_size || "—"} filled:${o.filled_size || "0"}`
        );
      ...
      case "market_cancel_orders": {
        const credsErr = requireMarketCreds();
        if (credsErr) return credsErr;
        const { order_ids } = args as { order_ids: string[] };
        const data = await advTradeCall("/api/v3/brokerage/orders/batch_cancel", "POST", {
          order_ids,
        });
        const results = (data.results as Array<Record<string, unknown>>) || [];
        const lines = results.map(
          (r) => `  ${r.order_id}: ${r.success ? "✓ cancelled" : `✗ failed — ${r.failure_reason}`}`
        );
      ...
      case "market_get_fills": {
        const credsErr = requireMarketCreds();
        if (credsErr) return credsErr;
        const { product_id, order_id, limit = 25 } = args as {
          product_id?: string;
          order_id?: string;
          limit?: number;
        };
        ...
        if (fills.length === 0) return ok("No fills found.");
        const lines = fills.map(
          (f) =>
            `  ${f.trade_time} | ${f.product_id} | ${f.side} | ${f.size} @ ${f.price} | fee:${f.commission}`
        );
...
export const marketModule: ToolModule = { definitions, handle };
