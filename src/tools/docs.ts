// Docs tools — fetch reference documentation for x402 extensions and Farcaster Mini Apps
import type { ToolModule, ToolDef, McpResult } from "../types.js";
import { ok, err } from "../types.js";

const MAX_DOC_LENGTH = 20000;

const DOC_PAGES: Record<string, { url: string; title: string }> = {
...
const definitions: ToolDef[] = [
  {
    name: "docs_fetch",
    description:
      "Fetch reference documentation for x402 protocol extensions or Farcaster Mini Apps. Use this to look up the latest specs for x402 payment idempotency, signed offers/receipts, gas sponsoring extensions, or Farcaster Mini Apps agent guidance and full LLM documentation.",
    inputSchema: {
      type: "object",
      properties: {
        topic: {
          ...
          description:
            "Documentation topic to fetch. Options: " +
...
async function handle(name: string, args: Record<string, unknown>): Promise<McpResult> {
  if (name !== "docs_fetch") throw new Error(`Unknown docs tool: ${name}`);
  ...
  try {
    ...
    return err(`Error fetching docs: ${(e as Error).message}`);
  }
}

export const docsModule: ToolModule = { definitions, handle };
