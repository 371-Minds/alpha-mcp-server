...
import { PythActionProvider, MessariActionProvider, FarcasterActionProvider, MorphoActionProvider, } from "@coinbase/agentkit";
// Helper functions to get singleton mock instances from the factory closure
function getPythMock() {
    return new PythActionProvider();
}
function getMessariMock() {
    return new MessariActionProvider();
}
function getFarcasterMock() {
    return new FarcasterActionProvider();
}
function getMorphoMock() {
    return new MorphoActionProvider();
}
// ---------------------------------------------------------------------------
// Module structure
// ---------------------------------------------------------------------------
...
describe("defi_farcaster_post — happy path", () => {
    ...
    it("passes channelId when channel is provided", async () => {
        ...
        expect(getFarcasterMock().postCast).toHaveBeenCalledWith(expect.objectContaining({ channelId: "defi" }));
    });
});
...
describe("bazaar_list", () => {
    ...
    it("returns error when both endpoints fail", async () => {
        globalThis.fetch = mock().mockResolvedValue({ ok: false, status: 503 });
        const result = await defiModule.handle("bazaar_list", {});
        expect(result.isError).toBe(true);
...
//# sourceMappingURL=defi.test.js.map