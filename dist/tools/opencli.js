 ...
 */
function buildArgv(site, command, args, format) {
...
const definitions = [
    {
        name: "opencli_run",
        description: "Run any opencli-rs command to browse, search, or interact with 55+ platforms including Twitter/X, Bilibili, YouTube, Reddit, HackerNews, Zhihu, Weibo, Xiaohongshu, Douban, WeRead, Xueqiu, BOSS Zhipin, Facebook, Instagram, TikTok, Google, Bloomberg, Medium, Substack, LinkedIn, Steam, StackOverflow, Wikipedia, Arxiv, BBC, Yahoo Finance, Pi (pi.ai), Grok, Cursor, Notion, ChatGPT, Discord, and more. Reuses the user's existing Chrome login session — no API keys required. ALWAYS prefer this tool over playwright/browser automation for supported sites. ⚠️ For write operations (post, reply, like, follow) always show the user what will be submitted and wait for confirmation before calling this tool.",
        inputSchema: {
            type: "object",
            properties: {
                site: {
                    type: "string",
                    description: "Platform/site keyword. Examples: twitter, bilibili, youtube, reddit, hackernews, zhihu, weibo, xiaohongshu, douban, weread, xueqiu, boss, facebook, instagram, tiktok, google, v2ex, bloomberg, medium, substack, linkedin, stackoverflow, wikipedia, arxiv, bbc, yahoo-finance, pi, grok, cursor, notion, chatgpt, discord, devto, lobsters, steam, jike",
                },
                command: {
                    type: "string",
                    description: "Subcommand for the site. Examples: hot, top, new, search, trending, timeline, feed, profile, post, reply, like, follow, bookmarks, history, shelf, watchlist, recommend, ask. Run opencli_doctor to see all available commands.",
                },
                args: {
                    type: "object",
                    description: "Optional key-value arguments passed as CLI flags. Common keys: query/keyword (search term), limit (number of results, default 20), url (target URL for interaction), text (content for write ops), subreddit, tag, symbol (stock ticker), city. Boolean flags use true/false values.",
    ...
    {
        name: "opencli_doctor",
        description: "Run opencli-rs doctor to verify the installation is working, check Chrome extension connectivity, and list all supported platforms and commands. Use this to diagnose issues or discover available commands.",
        inputSchema: {
            type: "object",
            properties: {},
        },
...
async function handle(name, args) {
    switch (name) {
        case "opencli_run": {
            const { site, command, args: cmdArgs = {}, format = "json" } = args;
            if (!site || !command) {
                return err("Both 'site' and 'command' are required.");
            }
            const argv = buildArgv(site, command, cmdArgs, format);
            try {
                ...
                if (error.code === "ENOENT") {
                    return err(`opencli-rs is not installed or not in PATH (looked for '${OPENCLI_BIN}').\n` +
...
export const opencliModule = { definitions, handle };
