...
async function commerceCall(endpoint, method, body) {
...
const definitions = [
    {
        ...
        description: "Create a Coinbase Commerce payment charge (invoice). Returns a hosted payment URL that customers can visit to pay with crypto. Requires COINBASE_COMMERCE_API_KEY env var.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "pay_get_charge",
        description: "Get the status and details of a Coinbase Commerce charge by its ID. Requires COINBASE_COMMERCE_API_KEY.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Create a Coinbase Commerce checkout session (reusable payment page). Requires COINBASE_COMMERCE_API_KEY.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Resolve (manually mark as complete) or void (cancel) an unresolved Coinbase Commerce charge. Requires COINBASE_COMMERCE_API_KEY.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "pay_x402_check",
        description: "Check whether a URL supports x402 autonomous payments by sending a probe request and inspecting the 402 Payment Required response headers.",
        inputSchema: {
            type: "object",
            properties: {
        ...
        switch (name) {
            case "pay_create_charge": {
                if (!process.env.COINBASE_COMMERCE_API_KEY)
                    return missingEnv("COINBASE_COMMERCE_API_KEY");
                const { name: chargeName, description, amount, currency = "USD", redirect_url, cancel_url, metadata } = args;
                const payload = {
                    name: chargeName,
                    description,
                ...
                return ok(`Charge created!\nID: ${chargeId}\nCode: ${code}\nHosted URL: ${hosted}\nAmount: ${amount} ${currency}\nStatus: ${data.timeline?.[0]?.status || "NEW"}`);
            }
            case "pay_get_charge": {
                if (!process.env.COINBASE_COMMERCE_API_KEY)
                    return missingEnv("COINBASE_COMMERCE_API_KEY");
                ...
                if (pricing?.local)
                    text += `Price: ${pricing.local.amount} ${pricing.local.currency}\n`;
            ...
            case "pay_create_checkout": {
                if (!process.env.COINBASE_COMMERCE_API_KEY)
                    return missingEnv("COINBASE_COMMERCE_API_KEY");
                const { name: checkoutName, description, amount, currency = "USD", pricing_type = "fixed_price", } = args;
                const payload = {
                    name: checkoutName,
                    description,
                    pricing_type,
            ...
            case "pay_list_events": {
                ...
                const lines = events.map((e) => {
                    const data = e.data;
                    return `  ${e.created_at} | ${e.type} | charge: ${data?.id || "—"}`;
                });
...
export const paymentsModule = { definitions, handle };
