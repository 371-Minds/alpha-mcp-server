...
async function getCdpWalletProvider() {
...
const definitions = [
    {
        ...
        description: "Fetch the Pyth Network price feed ID for a given token symbol (e.g. BTC, ETH, SOL). Returns the on-chain feed ID used by Pyth oracles.",
        inputSchema: {
            type: "object",
            properties: {
                token_symbol: {
    ...
    },
    {
        ...
        description: "Fetch the current real-time price from Pyth Network for a given Pyth price feed ID. Use defi_pyth_price_feed to get the feed ID first.",
        inputSchema: {
            type: "object",
            properties: {
                price_feed_id: {
    ...
    },
    {
        ...
        description: "Ask a research question about crypto markets, protocols, tokens, or DeFi trends using Messari AI. Requires MESSARI_API_KEY env var.",
        inputSchema: {
            type: "object",
            properties: {
                question: {
    ...
    },
    {
        ...
        description: "Post a cast (message) to Farcaster social network. Requires NEYNAR_API_KEY and NEYNAR_SIGNER_UUID env vars.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Deposit assets into a Morpho vault to earn yield. Requires CDP_API_KEY_ID, CDP_API_KEY_SECRET env vars (CDP wallet).",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Withdraw assets from a Morpho vault. Requires CDP_API_KEY_ID and CDP_API_KEY_SECRET env vars.",
        inputSchema: {
            type: "object",
            properties: {
                vault_address: { type: "string", description: "The Morpho vault contract address (0x...)" },
    ...
    },
    {
        ...
        description: "Get a bridge quote and initiate a cross-chain token transfer using the Across Protocol. Queries the Across API for fees and routes.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "bazaar_list",
        description: "Browse registered x402-enabled services in the x402 Bazaar marketplace. Returns a list of APIs that accept autonomous micropayments.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "bazaar_call",
        description: "Call any x402-enabled service from the Bazaar with automatic USDC micropayment. Requires GPUBRIDGE_WALLET_KEY (EVM private key) for payment. The service URL must be x402-compatible.",
        inputSchema: {
            type: "object",
            properties: {
        ...
        switch (name) {
            ...
            case "defi_farcaster_post": {
                if (!process.env.NEYNAR_API_KEY)
                    return missingEnv("NEYNAR_API_KEY");
                if (!process.env.NEYNAR_SIGNER_UUID)
                    return missingEnv("NEYNAR_SIGNER_UUID");
                const { message, channel } = args;
                const fc = getFarcaster();
                const castArgs = { castText: message };
                if (channel)
                    castArgs.channelId = channel;
            ...
            case "defi_morpho_deposit": {
                ...
                const { vault_address, assets, receiver, token_address } = args;
            ...
            case "defi_morpho_withdraw": {
                ...
                if (!walletProvider)
                    return missingEnv("CDP_API_KEY_ID", "CDP_API_KEY_SECRET");
                const { vault_address, assets, receiver } = args;
                const morpho = new MorphoActionProvider();
            ...
            case "defi_across_bridge": {
                const { origin_chain_id, destination_chain_id, token, amount, recipient } = args;
                ...
                if (!res.ok)
                    return err(`Across API error: ${res.statusText}`);
            ...
            case "bazaar_list": {
                ...
                if (!res.ok) {
                    ...
                    if (!rootRes.ok)
                        return err(`x402 Bazaar unavailable (${res.status}). Visit https://bazaar.x402.org to browse services.`);
            ...
            case "bazaar_call": {
                ...
                if (!walletKey)
                    return missingEnv("GPUBRIDGE_WALLET_KEY");
                const { url, method = "GET", body, chain = "base" } = args;
...
export const defiModule = { definitions, handle };
