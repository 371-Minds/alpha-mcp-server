...
async function cbCall(endpoint, method, token, body) {
...
const definitions = [
    {
        name: "cb_oauth_url",
        description: "Generate a Coinbase OAuth2 authorization URL. Direct the user to this URL to grant access to their Coinbase account. Requires COINBASE_CLIENT_ID env var.",
        inputSchema: {
            type: "object",
            properties: {
                redirect_uri: {
                ...
                scopes: {
                    type: "array",
                    items: { type: "string" },
                    description: "OAuth2 scopes to request. Common: wallet:accounts:read, wallet:transactions:read, wallet:buys:create. Default: wallet:accounts:read,wallet:transactions:read",
                },
                state: {
                    type: "string",
    ...
    },
    {
        ...
        description: "Exchange a Coinbase OAuth2 authorization code for an access token. Store the returned access token as COINBASE_OAUTH_TOKEN. Requires COINBASE_CLIENT_ID and COINBASE_CLIENT_SECRET.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "cb_get_user",
        description: "Get the authenticated Coinbase user's profile (name, email, country, native currency). Requires COINBASE_OAUTH_TOKEN.",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "cb_get_accounts",
        description: "List all Coinbase wallet accounts with their balances (BTC, ETH, USDC, and all supported assets). Requires COINBASE_OAUTH_TOKEN.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Get transaction history for a specific Coinbase account. Requires COINBASE_OAUTH_TOKEN.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Get the current spot price for a crypto/fiat currency pair from Coinbase. No authentication required.",
        inputSchema: {
            type: "object",
            properties: {
                currency_pair: {
    ...
    },
    {
        ...
        description: "Get exchange rates for a base currency against all supported fiat and crypto currencies. No authentication required.",
        inputSchema: {
            type: "object",
            properties: {
                currency: {
    ...
    try {
        switch (name) {
            case "cb_oauth_url": {
                ...
                if (!clientId)
                    return missingEnv("COINBASE_CLIENT_ID");
                const { redirect_uri, scopes, state } = args;
            ...
            case "cb_exchange_token": {
                ...
                if (!clientId)
                    return missingEnv("COINBASE_CLIENT_ID");
                if (!clientSecret)
                    return missingEnv("COINBASE_CLIENT_SECRET");
            ...
            case "cb_get_accounts": {
                ...
                const lines = accounts
                    ...
                    .map((a) => {
                    const bal = a.balance;
                    const nativeBal = a.native_balance;
                ...
                if (lines.length > 0)
                    ...
                    text += "  All accounts have zero balance.";
                return ok(text);
            }
            case "cb_get_transactions": {
                ...
                if (!token)
                    return missingEnv("COINBASE_OAUTH_TOKEN");
                const { account_id, limit = 25 } = args;
                const resp = await cbCall(`/accounts/${account_id}/transactions?limit=${limit}`, "GET", token);
                const txns = resp.data || [];
                if (txns.length === 0)
                    return ok("No transactions found for this account.");
                const lines = txns.map((t) => {
                    const amount = t.amount;
                    const native = t.native_amount;
            ...
            case "cb_get_exchange_rates": {
                ...
                if (!resp.ok)
                    return err(`Failed to get exchange rates (${resp.status})`);
                const data = (await resp.json()).data;
...
export const portfolioModule = { definitions, handle };
